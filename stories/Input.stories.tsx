...
const meta = {
  ...
  argTypes: {
    ...
    type: {
      control: 'select',
      options: ['text', 'password', 'email', 'number', 'tel', 'url', 'search', 'date', 'datetime-local', 'time', 'month', 'week'],
    },
    disabled: {
      control: 'boolean',
    },
    ...
    return (
      <div style={{ display: 'flex', 'flex-direction': 'column', gap: '1rem' }}>
        <Input 
          ...
          color="primary"
        />
        <div style={{ padding: '1rem', 'background-color': 'var(--fallback-b2,oklch(var(--b2)/1))', 'border-radius': '0.5rem' }}>
          <p style={{ 'margin-bottom': '0.5rem', color: 'var(--fallback-bc,oklch(var(--bc)/1))' }}>
          ...
          <p style={{ 
            ...
            {inputValue() || <em style={{ opacity: 0.5 }}>Your text will appear here...</em>}
          </p>
          <p style={{ 'margin-top': '0.5rem', 'font-size': '0.875rem', opacity: 0.7, color: 'var(--fallback-bc,oklch(var(--bc)/1))' }}>
...
export const MultipleInputsSync: Story = {
  render: () => {
    ...
    return (
      <div style={{ display: 'flex', 'flex-direction': 'column', gap: '1rem' }}>
        <h3 style={{ 'font-weight': 'bold', color: 'var(--fallback-bc,oklch(var(--bc)/1))' }}>
  ...
  render: () => {
    ...
    return (
      <form onSubmit={handleSubmit} style={{ display: 'flex', 'flex-direction': 'column', gap: '1rem' }}>
        <div>
          <label style={{ display: 'block', 'margin-bottom': '0.25rem', color: 'var(--fallback-bc,oklch(var(--bc)/1))' }}>
            Name
          </label>
          <Input 
            value={name()}
        ...
        <div style={{ 
          ...
          <h4 style={{ 'font-weight': 'bold', 'margin-bottom': '0.5rem', color: 'var(--fallback-bc,oklch(var(--bc)/1))' }}>
            Form Preview:
          </h4>
          <p style={{ color: 'var(--fallback-bc,oklch(var(--bc)/1))' }}>Name: {name() || '(empty)'}</p>
          <p style={{ color: 'var(--fallback-bc,oklch(var(--bc)/1))' }}>Email: {email() || '(empty)'}</p>
          <p style={{ color: 'var(--fallback-bc,oklch(var(--bc)/1))' }}>Message: {message() || '(empty)'}</p>
        </div>
      </form>
    );
  },
