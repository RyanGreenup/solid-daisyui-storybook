    ...
    const generateMathData = createMemo(() => {
      ...
      for (let i = 0; i < points; i++) {
        ...
        switch (functionType()) {
          ...
          case "square":
            y =
              amplitude() * Math.sign(Math.sin(frequency() * x + phase())) +
              offset();
            break;
          case "sawtooth":
            y =
              amplitude() *
                (2 *
                  ((frequency() * x + phase()) / (2 * Math.PI) -
                    Math.floor(
                      (frequency() * x + phase()) / (2 * Math.PI) + 0.5,
                    ))) +
              offset();
            break;
          default:
    ...
    const chartData = createMemo(() => {
      ...
      const getColor = () => {
        switch (functionType()) {
          ...
          case "cosine":
            return { border: "rgb(34, 197, 94)", bg: "rgba(34, 197, 94, 0.1)" };
          case "tangent":
            return { border: "rgb(239, 68, 68)", bg: "rgba(239, 68, 68, 0.1)" };
          case "square":
            return {
          ...
          <div class="flex items-center justify-between w-full">
            ...
            <div class="flex items-center gap-2">
              <span class="badge badge-secondary badge-sm">
                {generateMathData().points.length} rows
              </span>
      ...
      <div class="space-y-6">
        <Fieldset>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
              <div>
                <Label for="phase">Phase Shift: {phase().toFixed(1)}</Label>
                <Range
                  id="phase"
                  min={0}
                  max={6.28}
              ...
              <div>
                <Label for="offset">Vertical Offset: {offset()}</Label>
                <Range
                  id="offset"
                  min={-3}
                  max={3}
              ...
              <div>
                <Label for="data-points">Data Points: {dataPoints()}</Label>
                <Range
                  id="data-points"
                  min={10}
                  max={200}
                  ...
                  value={dataPoints()}
                  onInput={(e) =>
                    setDataPoints(parseInt(e.currentTarget.value))
                  }
          ...
          <div class="mt-4 p-4 bg-info/10 border border-info/20 rounded-lg">
            ...
            <code class="text-sm">
              y = {amplitude()} × {functionType()}({frequency()}x +{" "}
              {phase().toFixed(1)}) + {offset()}
            </code>
          </div>
        </Fieldset>
      </div>
      ...
      return (
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            ...
            <div class="badge badge-outline">{dataPoints()} data points</div>
          </div>
          ...
          <Show
            ...
            fallback={
              <div class="flex items-center justify-center h-96 bg-base-200 rounded-lg">
                <div class="text-center space-y-4">
                  <RadialProgress
                    value={Math.round(tableProgress())}
                  ...
                  <div class="space-y-1">
                    ...
                    <p class="text-sm text-base-content/50">
                      Preparing {generateMathData().points.length} rows for
                      virtualization
                    </p>
                  </div>
                  <div class="text-xs text-base-content/40">
      ...
      <>
        ...
        <div class="alert alert-info">
          ...
          <div>
            ...
            <div class="text-sm">
              This chart updates automatically as you adjust the mathematical
              parameters in the controls section above. The visualization
              demonstrates SolidJS fine-grained reactivity with Chart.js
              integration.
            </div>
          </div>
        </div>
      ...
      <>
        ...
        <div class="alert alert-success">
          ...
          <div>
            ...
            <div class="text-sm">
              This table uses virtualization to efficiently render large
              datasets. You can filter, sort, and download the mathematical
              data. The table automatically updates when you change the function
              parameters.
            </div>
          </div>
        </div>
      ...
      <div class="max-w-6xl mx-auto space-y-6">
        <div class="text-center space-y-2">
          ...
          <p class="text-base-content/60 max-w-2xl mx-auto">
            This advanced example demonstrates SolidJS reactivity with
            mathematical functions, real-time charting, and virtualized data
            tables. All three accordion sections update automatically as you
        ...
        <div class="alert alert-warning">
          ...
          <div>
            ...
            <div class="text-sm">
              • All three accordion sections update automatically when controls
              change
              <br />
              • Chart.js integration with real-time data binding
              <br />
              • Virtualized table performance with thousands of data points
              <br />
              • Multiple accordion sections can be open simultaneously
              (collapsible=true)
              <br />• Demonstrates complex data flow and fine-grained reactivity
            </div>
          </div>
        </div>
      </div>
